...
const createIncidenceWidget = (widget, data, customLandkreisName) => {
    ...
    casesLandkreisLabel.font = Font.systemFont(data.landkreis.cases7_per_100k >= 100 ? 10 : 15)
    casesLandkreisLabel.textColor = Color.black()

    const landkreisNameLabel = widget.addText(generateLandkreisName(data, customLandkreisName))
    landkreisNameLabel.minimumScaleFactor = 0.5
// end createIncidenceWidget
} 
...
const createInfectionsWidget = (widget, data) => {
    ...
    casesDiffLabel.textColor = Color.black()
    casesDifferenceStack.addSpacer()
...
async function createWidget() {
    ...
    let customLandkreisName2;
    let customLandkreisName3;
    ...
    if (params && params[0] !== "INF") {
        ...
        location2 = {
            latitude: parseFloat(params[3]),
            longitude: parseFloat(params[4])
        }
        customLandkreisName2 = params[5]
        location3 = {
            latitude: parseFloat(params[6]),
            longitude: parseFloat(params[7])
        }
        customLandkreisName3 = params[8]
    }

    const widget = new ListWidget()
    
    let filem = FileManager.iCloud()
let path = filem.joinPath(filem.documentsDirectory(), filename)
filem.downloadFileFromiCloud(path)
console.log(filem.fileExists(path))
console.log(path)
widget.backgroundImage = filem.readImage(path)

    switch (widgetMode) {
        case WIDGET_MODE.INCIDENCE:
            const data = await loadData(location)
            const data2 = await loadData(location2)
            const data3 = await loadData(location3)
            ...
            if (data2){
            createIncidenceWidget(widget, data2, customLandkreisName2);}
            if (data3){
            createIncidenceWidget(widget, data3, customLandkreisName3);}
            break;
